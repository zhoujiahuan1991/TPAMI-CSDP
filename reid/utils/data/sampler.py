...
class RandomMultipleGallerySampler(Sampler):
    def __init__(self, data_source, num_instances=4):
        ...
        #         self.pid_index[pid].append(index)
            ...
            for index, (_, pid, cam,_,_,_) in enumerate(data_source):#img_path, pid, camid, domain-id, image-id, clean_pid
                self.index_pid[index] = pid
                self.pid_cam[pid].append(cam)
                self.pid_index[pid].append(index)
        #print(self.pid_cam)
    ...
    def __iter__(self):
        ...
        for kid in indices:
            ...
            except:#img_path, pid, camid, domain-id, image-id, clean_pid
                _, i_pid, i_cam, _ , _,_= self.data_source[i]
            # except:
