...
class SmoothlabelClassCrossEntropyLoss(nn.Module):
    ...
    def forward(self, input, target, weights=None):
        ...
        if self.aggregate == 'sum':
            ...
            return loss
class Balance_ClassCrossEntropyLoss(nn.Module):
    def __init__(self,num_classes):
        super(Balance_ClassCrossEntropyLoss, self).__init__()
        self.num_classes=num_classes
    def forward(self, input, target):
        scores_one_hot = torch.zeros(len(target),self.num_classes).cuda()
        scores_one_hot=scores_one_hot.scatter_(1,target.unsqueeze(1),1)

        return -(F.softmax(input, 1) * F.log_softmax(scores_one_hot, dim=1)).sum(1).mean()
